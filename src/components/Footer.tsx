"use client";

import Image from "next/image";
import { useLanguage } from "@/context/LanguageProvider";

export default function Footer() {
  const { lang, dict } = useLanguage();
  const year = new Date().getFullYear();

  const handleScroll = (id: string) => {
    const el = document.querySelector(id);
    if (el) el.scrollIntoView({ behavior: "smooth" });
  };

  return (
    <footer className="relative bg-brand-900 text-white overflow-hidden">
      {/* Pattern overlay */}
      <div
        className="absolute inset-0 opacity-[0.04]"
        style={{
          backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M30 0L60 30L30 60L0 30Z' fill='none' stroke='%23ffffff' stroke-width='1'/%3E%3C/svg%3E")`,
          backgroundSize: "60px 60px",
        }}
      />

      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-10">
          {/* Brand */}
          <div>
            <div className="flex items-center gap-3 mb-4">
              <div className="relative w-12 h-12 rounded-xl overflow-hidden">
                <Image
                  src="/images/logo.png"
                  alt="Pizza Pan"
                  fill
                  className="object-cover"
                  sizes="48px"
                />
              </div>
              <span
                className="text-2xl font-bold"
                style={{ fontFamily: "var(--font-inter)" }}
              >
                Pizza Pan
              </span>
            </div>
            <p className="text-white/60 text-sm leading-relaxed max-w-xs">
              {lang === "ar"
                ? "عِشره بقالها سنين ومكملين. أحلى بيتزا، كريب، فطير وأكلات تانية كتير!"
                : "10 years and still going strong. The best pizza, crepe, pies and much more!"}
            </p>
          </div>

          {/* Quick Links */}
          <div>
            <h4 className="font-bold text-lg mb-4">
              {lang === "ar" ? "روابط سريعة" : "Quick Links"}
            </h4>
            <ul className="space-y-2">
              {[
                { href: "#home", label: dict.nav.home },
                { href: "#menu", label: dict.nav.menu },
                { href: "#branches", label: dict.nav.branches },
                { href: "#contact", label: dict.nav.contact },
              ].map((link) => (
                <li key={link.href}>
                  <button
                    onClick={() => handleScroll(link.href)}
                    className="text-white/60 hover:text-gold-400 transition-colors text-sm"
                  >
                    {link.label}
                  </button>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h4 className="font-bold text-lg mb-4">
              {dict.contact.title}
            </h4>
            <div className="space-y-3">
              <a
                href="tel:15616"
                className="flex items-center gap-2 text-white/60 hover:text-gold-400 transition-colors text-sm"
              >
                <span className="material-icons-outlined text-lg">phone</span>
                15616
              </a>
              <div className="flex items-start gap-2 text-white/60 text-sm">
                <span className="material-icons-outlined text-lg mt-0.5">location_on</span>
                <span>{dict.branches.locations.kafrElSheikh.address}</span>
              </div>
              <div className="flex gap-3 mt-4">
                <a
                  href="https://www.facebook.com/Pizza.Pan.Kfs"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-blue-600 transition-colors"
                >
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                  </svg>
                </a>
                <a
                  href="https://instagram.com/pizza___pan"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-gradient-to-r hover:from-purple-600 hover:to-pink-500 transition-colors"
                >
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                  </svg>
                </a>
              </div>
            </div>
          </div>
        </div>

        {/* Divider */}
        <div className="border-t border-white/10 pt-6 text-center">
          <p className="text-white/40 text-sm">
            © {year} Pizza Pan. {dict.footer.rights}.
          </p>
          <p className="text-white/30 text-xs mt-1">
            {dict.footer.madeWith}
          </p>
        </div>
      </div>
    </footer>
  );
}
